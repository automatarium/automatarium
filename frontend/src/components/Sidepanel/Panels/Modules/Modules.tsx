...
import { toModuleProject } from "../../../../util/toModuleProject";
...
const Modules = () => {
 ...
 const saveModule = () => {
  ...
  if (!project) return; //prevent spreading null

  updateProjectToModule({
    ...project,
    meta: {
      ...project.meta,
      dateEdited: Date.now(), // preserves automatariumVersion, dateCreated, version
    },
  });
  ...
  const { register, handleSubmit } = useForm<{ questionType: string }>({
...
const newModuleProject = createNewModuleProject(data.questionType as any);
  ...
  const confirmDeleteQuestion = () => {
    ...
    if (projectToDelete._id === currentProject._id && currentModule) {
      const remaining = currentModule.projects.filter(
  ...
  const handleDrop = (dropIndex: number) => {
    ...
    const updated = [...currentModule.projects];
    const [moved] = updated.splice(draggedIndex, 1);
    updated.splice(dropIndex, 0, moved);
    setModuleProjects(updated);
        ...
        <>
          ...
          <Wrapper>
            <Preference label={t("module_panel.open_questions")}>
              <Switch
                type="checkbox"
                checked={showModuleWindow}
                onChange={() => setShowModuleWindow(!showModuleWindow)}
              />
            </Preference>
          </Wrapper>
              ...
              <tbody>
                {currentModule.projects.map((q, index) => (
                  <tr
                    ...
                    style={{
                      backgroundColor:
                        currentProject && currentProject._id === q._id
                          ? "var(--toolbar)"
                          : "transparent",
                    }}
                  ...
                  >
                    <td onClick={() => handleOpenQuestion(q)}>
                      {t("module_panel.question_id", { id: index + 1 })}
                    </td>
                    <td>
                      <EditButton onClick={() => handleEditQuestion(q)}>
                        {t("menus.edit")}
                      </EditButton>
                      <RemoveButton
