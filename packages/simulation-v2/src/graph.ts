...
export class GraphNode {
    ...
    constructor(state: FSAGraphState, transition: Transition | null = null, parent: GraphNode | null = null) {
        this.m_state = state;
...
export class FSAGraphProblem {
    ...
    public getSuccessors(state: FSAGraphState) {
        ...
        for (const transition of transitions) {
            ...
            if (nextState === undefined || !lambdaTransition || !transition.read.includes(state.read[0])) {
                continue;
            }
