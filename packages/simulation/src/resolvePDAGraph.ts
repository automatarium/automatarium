import {ReadSymbol, PDAGraph, UnresolvedPDAGraph} from './types'
...
export const resolvePDAGraph = (graph: UnresolvedPDAGraph): PDAGraph => {
  ...
  const transitions = graph.transitions
    ...
    .map(transition => ({...transition, readSymbol: expandReadSymbols(transition.readSymbol), 
        pushSymbol: expandReadSymbols(transition.pushSymbol), popSymbol: expandReadSymbols(transition.popSymbol)}))
