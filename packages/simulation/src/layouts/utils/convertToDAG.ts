 ...
 */
export const adjacencyListToTransitions = (
  graph: AutomataProjectGraph,
  adjacencyList: AdjacencyList
): BaseAutomataTransition[] => {
  ...
  adjacencyList.forEach((adjList, k) => {
    adjList.forEach(adj => {
      ...
      const transition = graph.transitions.find(t => t.from === from && t.to === to)
...
// Make graph acyclic
export const convertToDAG = <T extends AutomataProjectGraph>(
  graph: T
): [T, AdjacencyList] => {
  /**
   * Priority:
   ...
   */
  const getInitialState = (graph: AutomataProjectGraph): number => {
    // Choose source node
   ...
   */
  const getStateCycles = (
    currentState: number,
    problem: DetectCyclesProblem,
    visited: number[],
    frontier: number[]
  ): CyclesCounter => {
    ...
    const frontierAdditions: number[] = []
    ...
    if (nextAdjList) {
      nextAdjList.forEach(p => {
        ...
        if (nextVisited.includes(id)) {
          ...
          problem.cyclesCounter.set(
            edgeKey,
            (problem.cyclesCounter.get(edgeKey) ?? 0) + 1
          )
        } else {
  ...
  const getMaximalCycler = (cyclesCounter: CyclesCounter): Edge | null => {
    const maxPair = [null, 0] as [Edge | null, number]
  ...
  const reverseEdge = (
    ...
    const transitionsToReverse = graph.transitions.filter(
      t => t.to === to && t.from === from
    )

    const transitionsNotToReverse = graph.transitions.filter(
      t => t.to !== to || t.from !== from
    )
    ...
    graph.transitions = [
      ...transitionsToReverse,
      ...transitionsNotToReverse
    ] as typeof graph.transitions

    // Update adjacency list
    const existingEdge = edges.get(from)?.find(e => e.id === to)

    if (!existingEdge) {
      return
    }

    const toReverseWeight = structuredClone(existingEdge)
  ...
  const graphClone = structuredClone(graph) as T
  ...
  for (const t of graphClone.transitions.filter(t => t.from !== t.to)) {
    if (edges.has(t.from)) {
      if (edges.get(t.from)?.some(p => p.id === t.to)) {
        const edge = edges.get(t.from)?.find(p => p.id === t.to)
  ...
  graphClone.transitions = adjacencyListToTransitions(
    graphClone,
    edges
  ) as typeof graphClone.transitions
  ...
  while (!done) {
    ...
    cloneStates.forEach(from => {
      cloneStates.forEach(to => {
        if (from !== to) {
          cyclesCounter.set([from.id, to.id].join(','), 0)
        }
      })
    })
